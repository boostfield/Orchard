...
namespace M33.Layout.Controllers {
    ...
    public  class AdminController : Controller, IUpdateModel {
        ...
        private IEnumerable<FileModel> EnumerateFiles(string filepath)
        {
            ...
            //Support for edit Razor partials. Loads files from Parts folder.
            if (TypeOfFile == FileType.RAZOR)
            {
                string parts =System.Web.HttpContext.Current.Server.MapPath(string.Format("~/Themes/{0}/{1}/Parts", _siteThemeService.GetCurrentThemeName(),folder));
                if(Directory.Exists(parts)){
                    var partials = Directory.EnumerateFiles(parts, extension);
                    foreach (string f in partials)
                        docs.Add(new FileModel { Name = new FileInfo(f).Name, Path = f, Current = (f == filepath) });
                }
            }
